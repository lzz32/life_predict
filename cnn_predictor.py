...
基于lifePredict_test.py，提供CNN模型的训练和预测功能：
- 读取CSV数据
- 8/2数据集划分
- 训练CNN模型（AC准确率>90%或迭代次数不超过100次停止）
- 向后预测20个数据点
- 保存预测结果和组合数据
"""
...
def train_cnn_model(preprocessed_data: dict,
                    max_epochs: int = 100,
                    ...
                    logger=None) -> dict:
    """
    使用CNN进行训练，早停条件：AC >= ac_threshold 或 迭代达到 max_epochs
...
def forecast_cnn(model: nn.Module,
    ...
    """
    基于训练好的CNN模型，迭代式地向前滚动预测 steps_ahead 个点
...
def run_cnn_prediction(input_csv: str,
                       output_dir: str,
    ...
    logger = setup_logging(output_dir, "cnn_prediction_log.txt")
    start_time = time.time()

    logger.info("=== CNN趋势预测流程开始 ===")
    ...
    try:
        ...
        # 3. 训练模型
        logger.info("开始训练CNN模型（AC早停）...")
        train_result = train_cnn_model(
            preprocessed,
        ...
        # 7. 保存模型
        model_file = os.path.join(output_dir, "cnn_model.pth")
        torch.save({
        ...
        logger.info(f"CNN模型已保存：{model_file}")

        elapsed = time.time() - start_time
        logger.info(f"总耗时：{elapsed:.2f} 秒")
        logger.info("=== CNN趋势预测流程结束 ===")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 3:
        ...
        print("用法: python cnn_predictor.py <input_csv> <output_dir> [ci_column]")
        sys.exit(1)
