...
class TrendLSTMModel(nn.Module):
    ...
    def __init__(self, input_size=1, hidden_size=32, num_layers=2, output_size=1, dropout=0.3):
        super(TrendLSTMModel, self).__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers
...
def setup_logging(output_dir):
    """设置日志记录"""
    log_file = os.path.join(output_dir, f"LSTM_life_prediction_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
...
def trend_prediction_lstm(df, output_dir):
    """CI值_ewma趋势预测LSTM模块"""
    ...
    while current_test_start <= test_end:  # 修改条件，包含最后一个点
        ...
        criterion = AsymmetricLoss(under_prediction_penalty=10.0)
    ...
    if len(all_actuals) > 0 and len(all_predictions) > 0:
        ...
        print(len(all_actuals), len(all_predictions))
...
# def improved_life_prediction_lstm():
def RULLSTM(input_file,output_dir,log_dir,ciname):
    # 记录开始时间
    ...
    # output_dir = r"D:\code_python\Project\25117DT\results\健康评估\交流发电机\RULLSTM"
    ...
    logger.info("开始LSTM寿命预测分析")
    logger.info(f"输入文件: {input_file}")
    logger.info(f"输出目录: {output_dir}")

    # 改进的LSTM参数
    window_size = 5  # 减小时间窗口大小
    ...
    try:
        ...
        ci_data, sigma_2, sigma_3, sigma_6, sigma_7, trend_actuals, trend_predictions, trend_ac = trend_prediction_lstm(
            df, output_dir)
        ...
        for epoch in range(epochs):
            ...
            if val_loss.item() < best_val_loss:
                ...
                torch.save(model.state_dict(), os.path.join(output_dir, 'improved_lstm_model.pth'))
        ...
        # 加载最佳模型
        model.load_state_dict(torch.load(os.path.join(output_dir, 'improved_lstm_model.pth')))
        ...
        # 16. 保存结果到CSV文件
        output_file = os.path.join(output_dir, "改进_LSTM_索引乘CI_寿命预测结果.csv")
        ...
        # 保存合并的图片
        combined_plot_file = os.path.join(output_dir, "合并_LSTM_寿命预测结果图.png")
        plt.savefig(combined_plot_file, dpi=300, bbox_inches='tight')
        logger.info(f"合并预测结果图已保存到: {combined_plot_file}")

        # 18. 保存模型参数和评估结果
        summary_file = os.path.join(output_dir, "改进_LSTM_索引乘CI_模型评估摘要.txt")
        with open(summary_file, 'w', encoding='utf-8') as f:
            f.write("改进的LSTM寿命预测模型评估摘要\n")
            f.write("=" * 60 + "\n")
            ...
            f.write(f"LSTM隐藏层大小: {hidden_size}\n")
        ...
        # 保存完整模型
        model_file = os.path.join(output_dir, "improved_lstm_index_ci_complete_model.pth")
        torch.save({
        ...
        logger.info(f"完整改进LSTM模型已保存到: {model_file}")
        ...
        # 保存准确率到csv
        output_data = {'算法模型': 'RULLSTM', 'CI值趋势预测AC': trend_ac, '运行时间/s': total_time}
...
if __name__ == "__main__":
    ...
    output_dir = r"D:\Python\25117DT\results\健康评估\交流发电机\RULLSTM"
    log_dir = output_dir  # 日志文件保存在输出目录
    ciname = 'om1'
    RULLSTM(input_file, output_dir, log_dir, ciname)